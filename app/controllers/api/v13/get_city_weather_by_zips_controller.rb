class Api::V13::GetCityWeatherByZipsController < Api::ApiController
  resource_description do
    name 'V13::GetCityWeatherByZips'
    short 'V13::GetCityWeatherByZips'
    path '/v1/get_city_weather_by_zips.json'
    version '13'
    formats ['json']
    param :id, Integer, :desc => "V13::GetCityWeatherByZip ID", :required => false
    param :get_city_weather_by_zip, Hash, :desc => "V13::GetCityWeatherByZip parameters for all methods" do
  ...
  api :GET, "/v13/get_city_weather_by_zips.json", "Show all get_city_weather_by_zips based on a query scope"
  def index
  ...
  api :GET, "/v13/get_city_weather_by_zips/:id.json", "Show get_city_weather_by_zip"
  def show
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  api :GET, "/v13/get_city_weather_by_zips/new.json", "Instantiate a new get_city_weather_by_zip"
  def new
    @get_city_weather_by_zip = V13::GetCityWeatherByZip.new
    respond_with @get_city_weather_by_zip
  end

  # POST /get_city_weather_by_zips.json
  api :POST, "/v13/get_city_weather_by_zips.json", "Create a new get_city_weather_by_zip"
  def create
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  api :PUT, "/v13/get_city_weather_by_zips/:id.json", "Update an existing get_city_weather_by_zip"
  def update
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  api :DELETE, "/v13/get_city_weather_by_zips/:id.json", "Delete an existing get_city_weather_by_zip"
  def destroy
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  def scope_named(name)
    scope = 'all' unless ::V13::GetCityWeatherByZip.respond_to?(name)
    if name == 'all'

      scope = ::V13::GetCityWeatherByZip.all
      scope = scope.all(offset: params[:offset].to_i) if params[:offset]
      scope = scope.all(limit: params[:limit].to_i) if params[:limit]

    elsif name == 'count'
      scope = ::V13::GetCityWeatherByZip.count
    else
      query_params = params[:query].is_a?(String) ? ( JSON.parse(params[:query]) rescue nil ) : params[:query]
      scope = ::V13::GetCityWeatherByZip.send(name, (query_params || {}), {}, params[:offset], params[:limit])
    end
    scope
  end
  ...
  include Api::V13::Custom::GetCityWeatherByZipsController
end
