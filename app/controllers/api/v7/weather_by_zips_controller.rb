class Api::V7::WeatherByZipsController < Api::ApiController
  resource_description do
    name 'V7::WeatherByZips'
    short 'V7::WeatherByZips'
    path '/v1/weather_by_zips.json'
    version '7'
    formats ['json']
    param :id, Integer, :desc => "V7::WeatherByZip ID", :required => false
    param :weather_by_zip, Hash, :desc => "V7::WeatherByZip parameters for all methods" do
  ...
  api :GET, "/v7/weather_by_zips.json", "Show all weather_by_zips based on a query scope"
  def index
  ...
  api :GET, "/v7/weather_by_zips/:id.json", "Show weather_by_zip"
  def show
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  api :GET, "/v7/weather_by_zips/new.json", "Instantiate a new weather_by_zip"
  def new
    @weather_by_zip = V7::WeatherByZip.new
    respond_with @weather_by_zip
  end

  # POST /weather_by_zips.json
  api :POST, "/v7/weather_by_zips.json", "Create a new weather_by_zip"
  def create
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  api :PUT, "/v7/weather_by_zips/:id.json", "Update an existing weather_by_zip"
  def update
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  api :DELETE, "/v7/weather_by_zips/:id.json", "Delete an existing weather_by_zip"
  def destroy
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  include Api::V7::Custom::WeatherByZipsController
end
