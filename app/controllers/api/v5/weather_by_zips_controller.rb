class Api::V5::WeatherByZipsController < Api::ApiController
  resource_description do
    name 'V5::WeatherByZips'
    short 'V5::WeatherByZips'
    path '/v1/weather_by_zips.json'
    version '5'
    formats ['json']
    param :id, Integer, :desc => "V5::WeatherByZip ID", :required => false
    param :weather_by_zip, Hash, :desc => "V5::WeatherByZip parameters for all methods" do
  ...
  api :GET, "/v5/weather_by_zips.json", "Show all weather_by_zips based on a query scope"
  def index
  ...
  api :GET, "/v5/weather_by_zips/:id.json", "Show weather_by_zip"
  def show
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  api :GET, "/v5/weather_by_zips/new.json", "Instantiate a new weather_by_zip"
  def new
    @weather_by_zip = V5::WeatherByZip.new
    respond_with @weather_by_zip
  end

  # POST /weather_by_zips.json
  api :POST, "/v5/weather_by_zips.json", "Create a new weather_by_zip"
  def create
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  api :PUT, "/v5/weather_by_zips/:id.json", "Update an existing weather_by_zip"
  def update
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  api :DELETE, "/v5/weather_by_zips/:id.json", "Delete an existing weather_by_zip"
  def destroy
    reject_as_unauthorized and return if requires_authentication? and !authenticated?
  ...
  include Api::V5::Custom::WeatherByZipsController
end
