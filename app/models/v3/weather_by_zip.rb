class V3::WeatherByZip
  ...
  storage_names[:'1743'] = 'get_city_weather_by_zip'
  ...
  property :get_city_weather_by_zip, Text, field: "GetCityWeatherByZIP", key: false, required: false, lazy: false

  property :get_city_weather_by_zip_response, Text, field: "GetCityWeatherByZIPResponse", key: false, required: false, lazy: false
  ...
  attr_accessible :get_city_weather_by_zip, :get_city_weather_by_zip_response, as: :"System Admin"
  # Default accessible scopes when nothing is defined
  attr_accessible :get_city_weather_by_zip, :get_city_weather_by_zip_response, as: :"Unauthenticated Default on create"
  attr_accessible :get_city_weather_by_zip, :get_city_weather_by_zip_response, as: :"Unauthenticated Default on update"
  attr_accessible :get_city_weather_by_zip, :get_city_weather_by_zip_response, as: :"Authenticated Without Role Default on create"
  attr_accessible :get_city_weather_by_zip, :get_city_weather_by_zip_response, as: :"Authenticated Without Role Default on update"


  def self.create(attrs = {}, as: :default)
    resource = new
    resource.assign_attributes(V3::WeatherByZip.cast_attributes(attrs), as: as )
    resource.save
    resource
  end
  ...
  def initialize(attrs = {}, options = {})
    super({})
    self.assign_attributes(V3::WeatherByZip.cast_attributes(attrs), as: options[:as] )
  end
  ...
  def self.cast_attributes(attributes = {})
    ...
    attributes.each_pair do |key,value|
      if key.is_a?(::DataMapper::Associations::Relationship)
      ...
      elsif V3::WeatherByZip.properties[key.to_sym] && value
        case V3::WeatherByZip.properties[key.to_sym].primitive.name
        when 'Integer'
        ...
        when 'BSON::ObjectId'
          ...
          new_attributes[key.to_sym] = (value.is_a?(V3::WeatherByZip.properties[key.to_sym].primitive) ? value : nil )
        end
      else
  ...
  include V3::Custom::WeatherByZip
end
